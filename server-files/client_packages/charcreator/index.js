...
function updateParents() {
    ...
    // shape
    Data.mothers[motherItem.Index], Data.fathers[fatherItem.Index], 0, 
    // skin
    Data.mothers[motherItem.Index], Data.fathers[fatherItem.Index], 0, 
    // mixes
    similarityItem.Index * 0.01, skinSimilarityItem.Index * 0.01, 0.0, false);
}
...
function fillHairMenu() {
    hairItem = new UIMenuListItem("Hair", "Your character's hair.", new ItemsCollection(Data.hairList[currentGender].map(function (h) { return h.Name; })));
...
function resetParentsMenu(refresh) {
...
function resetFeaturesMenu(refresh) {
    if (refresh === void 0) { refresh = false; }
...
function resetAppearanceMenu(refresh) {
    if (refresh === void 0) { refresh = false; }
...
function resetHairAndColorsMenu(refresh) {
    if (refresh === void 0) { refresh = false; }
    hairItem.Index = 0;
...
// color arrays
var hairColors = [];
for (var i = 0; i < Data.maxHairColor; i++)
    hairColors.push(i.toString());
var blushColors = [];
for (var i = 0; i < Data.maxBlushColor; i++)
    blushColors.push(i.toString());
var lipstickColors = [];
for (var i = 0; i < Data.maxLipstickColor; i++)
    lipstickColors.push(i.toString());
// CREATOR MAIN
var creatorMainMenu = new Menu("Creator", "", new Point(50, 50));
var genderItem = new UIMenuListItem("Gender", "~r~Changing this will reset your customization.", new ItemsCollection(["Male", "Female"]));
...
var angles = [];
for (var i = -180.0; i <= 180.0; i += 5.0)
    angles.push(i.toFixed(1));
var angleItem = new UIMenuListItem("Angle", "", new ItemsCollection(angles));
creatorMainMenu.AddItem(angleItem);
var saveItem = new UIMenuItem("Save", "Save all changes.");
saveItem.BackColor = new Color(13, 71, 161);
saveItem.HighlightedBackColor = new Color(25, 118, 210);
creatorMainMenu.AddItem(saveItem);
var cancelItem = new UIMenuItem("Cancel", "Discard all changes.");
cancelItem.BackColor = new Color(213, 0, 0);
cancelItem.HighlightedBackColor = new Color(229, 57, 53);
creatorMainMenu.AddItem(cancelItem);
creatorMainMenu.ListChange.on(function (item, listIndex) {
    if (item == genderItem) {
...
creatorMainMenu.ItemSelect.on(function (item, index) {
    switch (index) {
        case 1:
        ...
        case 6:
            ...
            var appearanceData = [];
            for (var i = 0; i < appearanceItems.length; i++)
...
creatorMainMenu.MenuClose.on(function () {
    mp.events.callRemote("creator_Leave");
});
creatorMainMenu.Visible = false;
...
for (var i = 0; i <= 100; i++)
    similarities.push(i + "%");
var creatorParentsMenu = new Menu("Parents", "", new Point(50, 50));
var fatherItem = new UIMenuListItem("Father", "Your character's father.", new ItemsCollection(Data.fatherNames));
var motherItem = new UIMenuListItem("Mother", "Your character's mother.", new ItemsCollection(Data.motherNames));
var similarityItem = new UIMenuListItem("Resemblance", "Similarity to parents.\n(lower = feminine, higher = masculine)", new ItemsCollection(similarities));
var skinSimilarityItem = new UIMenuListItem("Skin Tone", "Skin color similarity to parents.\n(lower = mother's, higher = father's)", new ItemsCollection(similarities));
...
creatorParentsMenu.ItemSelect.on(function (item, index) {
    switch (item.Text) {
        case "Randomize":
...
creatorParentsMenu.ListChange.on(function (item, listIndex) {
    updateParents();
});
...
for (var i = -1.0; i <= 1.01; i += 0.01)
    features.push(i.toFixed(2));
var creatorFeaturesMenu = new Menu("Features", "", new Point(50, 50));
for (var i = 0; i < Data.featureNames.length; i++) {
...
creatorFeaturesMenu.ItemSelect.on(function (item, index) {
    switch (item.Text) {
        case "Randomize":
            for (var i = 0; i < Data.featureNames.length; i++) {
...
creatorFeaturesMenu.ListChange.on(function (item, listIndex) {
...
for (var i = 0; i <= 100; i++)
    opacities.push(i + "%");
var creatorAppearanceMenu = new Menu("Appearance", "", new Point(50, 50));
for (var i = 0; i < Data.appearanceNames.length; i++) {
    var items = [];
    for (var j = 0, max = mp.game.ped.getNumHeadOverlayValues(i); j <= max; j++)
    ...
    var tempAppearanceOpacityItem = new UIMenuListItem(Data.appearanceNames[i] + " Opacity", "", new ItemsCollection(opacities));
...
creatorAppearanceMenu.ItemSelect.on(function (item, index) {
    switch (item.Text) {
        case "Randomize":
            for (var i = 0; i < Data.appearanceNames.length; i++) {
...
creatorAppearanceMenu.ListChange.on(function (item, listIndex) {
...
var hairItem;
var hairColorItem;
var hairHighlightItem;
var eyebrowColorItem;
var beardColorItem;
var eyeColorItem;
var blushColorItem;
var lipstickColorItem;
var chestHairColorItem;
var creatorHairMenu = new Menu("Hair & Colors", "", new Point(50, 50));
fillHairMenu();
creatorHairMenu.ItemSelect.on(function (item, index) {
    switch (item.Text) {
        case "Randomize":
...
creatorHairMenu.ListChange.on(function (item, listIndex) {
    if (item == hairItem) {
        var hairStyle = Data.hairList[currentGender][listIndex];
...
// EVENTS
mp.events.add("toggleCreator", function (active, charData) {
    if (active) {
        ...
        if (charData) {
            ...
            similarityItem.Index = parseInt(String(charData.Parents.Similarity * 100));
            skinSimilarityItem.Index = parseInt(String(charData.Parents.SkinSimilarity * 100));
            updateParents();
            // features
            for (var i = 0; i < charData.Features.length; i++) {
                featureItems[i].Index = (charData.Features[i] * 100) + 100;
                updateFaceFeature(i);
            }
            // hair and colors s
            var hair = Data.hairList[currentGender].find(function (h) { return h.ID == charData.Hair.Hair; });
            ...
            updateHairAndColors();
            // appearance
            for (var i = 0; i < charData.Appearance.length; i++) {
