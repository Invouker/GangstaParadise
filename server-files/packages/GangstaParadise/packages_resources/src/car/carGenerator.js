"use strict";
var data = require("./CarGens_ZoneVehicles");
generateCar();
function generateCar() {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var model = data[i].models[Math.floor(Math.random() * data[i].models.length)];
        ...
        if (model === "mixer" || model === "mixer2" || model === "hauler" && (Math.floor(Math.random() * 20) >= 10))
            continue;
        var veh = mp.vehicles.new(model, new mp.Vector3(data[i].x, data[i].y, data[i].z), {
            heading: data[i].heading,
            numberPlate: getRandomNumberPlate(),
        });
        if (model === "taxi") {
            var primary = (Math.floor(Math.random() * 10) >= 5) ? 88 : 126;
