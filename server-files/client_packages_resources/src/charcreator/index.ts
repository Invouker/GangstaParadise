...
function colorForOverlayIdx(index: number) {
    let color;
...
function fillHairMenu() {
    hairItem = new UIMenuListItem("Hair", "Your character's hair.", new ItemsCollection(Data.hairList[currentGender].map((h:any) => h.Name)));
...
let currentGender:number = 0;
let creatorMenus:any = [];
let creatorCamera:any;
...
creatorMainMenu.ListChange.on((item:any, listIndex:any) => {
    if (item == genderItem) {
...
creatorMainMenu.ItemSelect.on((item:any, index:any) => {
    switch (index) {
        case 1:
...
creatorParentsMenu.ItemSelect.on((item:any, index:any) => {
    switch (item.Text) {
        case "Randomize":
...
creatorParentsMenu.ListChange.on((item:any, listIndex:any) => {
    updateParents();
});
...
creatorFeaturesMenu.ItemSelect.on((item:any, index:any) => {
    switch (item.Text) {
        case "Randomize":
            for (let i = 0; i < Data.featureNames.length; i++) {
...
creatorFeaturesMenu.ListChange.on((item:any, listIndex:any) => {
...
creatorAppearanceMenu.ItemSelect.on((item:any, index:any) => {
    switch (item.Text) {
        case "Randomize":
            for (let i = 0; i < Data.appearanceNames.length; i++) {
...
creatorAppearanceMenu.ListChange.on((item:any, listIndex:any) => {
...
// CREATOR HAIR & COLORS
let hairItem:any;
let hairColorItem:any;
let hairHighlightItem:any;
let eyebrowColorItem:any;
let beardColorItem:any;
let eyeColorItem:any;
let blushColorItem:any;
let lipstickColorItem:any;
let chestHairColorItem:any;

let creatorHairMenu = new Menu("Hair & Colors", "", new Point(50, 50));
fillHairMenu();

creatorHairMenu.ItemSelect.on((item:any, index:any) => {
    switch (item.Text) {
        case "Randomize":
...
creatorHairMenu.ListChange.on((item:any, listIndex:any) => {
    if (item == hairItem) {
...
mp.events.add("toggleCreator", (active, charData) => {
    if (active) {
        ...
        if (charData) {
            ...
            // hair and colors s
            let hair = Data.hairList[currentGender].find((h:any) => h.ID == charData.Hair.Hair);
